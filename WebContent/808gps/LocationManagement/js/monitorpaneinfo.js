...
monitorPaneInfo.prototype.addPanePosition = function(vehiIdno, data){
	if((typeof monitorStatus) != 'undefined' && monitorStatus != null && monitorStatus.isClickPosition) {
		...
		$('#state_position .maplngLat').attr('onclick', 'changeMapAddress(this,'+data.mapJingDu+','+data.mapWeiDu+',\''+vehiIdno+'\');');
	}else {
		...
		var position = '<span class="maplngLat" data-type="2" data-position="'+ pos2 +'" onclick="changeMapAddress(this,'+data.mapJingDu+','+data.mapWeiDu+',\''+vehiIdno+'\');" title="'+ pos1 +'">'+ pos1 +'</span>';
		$('#state_position div').html(position);
	}
}
